...
void Hostage::setMyHero(Hero* hero) { myHero = hero; }

void Hostage::setNextPoint(Point nextp) { nextPosition = nextp; }
...
int Hostage::getTotalClocksNeeded() { return totalClocksNeeded; }
...
void Hostage::setClockPassed(int clocknum) {
    ...
    (float)(totalClocksNeeded - clocknum) / (float)totalClocksNeeded;
...
bool Hostage::isSaved() { return myHero == NULL ? false : true; }
...
void Hostage::followHero() {
    ...
    if (clock % HOSTAGESPEED == 0) {
        ...
        if (nextPosition == this->getPosition()) return;
        clock = 0;
        subpath.clear();
        ...
        for (int i = 1; i <= HOSTAGESPEED; i++) {
            Point pathPoint =
            ((nextPosition - currentPos) / HOSTAGESPEED) * i + currentPos;
