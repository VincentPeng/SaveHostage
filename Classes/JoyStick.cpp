...
/**********
 Instantiate Joystick function
 
 dockName : Base background image
 ballName : Ball image
 ballradius : Ball radius
 movearearadius : Ball movement radius
 isfollowtouch : Is follow touch?
 iscanvisible  : Is visible?
 isautohide : Is auto hidden when no opperation
 hasanimation : Has animation
 **********/
JoyStick *JoyStick::createJoyStick(const string &dockName,
    ...
    if (joystick) {
        ...
        joystick->initWithBallRadius(ballradius, movearearadius, isfollowtouch,
...
void JoyStick::initWithBallRadius(int ballradius, int movearearadius,
...
bool JoyStick::containsTouchLocation(Touch *touch) {
    ...
    if (ActiveRadius > 0) {
        if (touchPoint.getDistance(this->getParent()->convertToWorldSpace(
                                                                          this->getPosition())) < ActiveRadius) {
            return true;
        }
    }
    if (ActiveRect.size.width > 0 && ActiveRect.size.height > 0) {
        if (touchPoint.x > ActiveRect.origin.x &&
            ...
            touchPoint.y < ActiveRect.origin.y + ActiveRect.size.height) {
            return true;
        }
    }
    return false;
...
void JoyStick::updateTouchPoint(Point touchPoint) {
    ...
    if (touchPoint.getDistance(Point(selfposition.x, selfposition.y)) >
        (MoveAreaRadius - BallRadius)) {
        Vec2 newv =
        Vec2(touchPoint.x - selfposition.x, touchPoint.y - selfposition.y)
        .getNormalized();
        CurrentPoint = Point::ZERO + newv * (float)(MoveAreaRadius - BallRadius);
    } else {
        CurrentPoint =
        Point(touchPoint.x - selfposition.x, touchPoint.y - selfposition.y);
    }
    Ball->setPosition(CurrentPoint);
    
    Angle = atan2(Ball->getPositionY(), Ball->getPositionX()) / (3.14159 / 180);
    Power = Ball->getPosition().getDistance(Point::ZERO) /
    (MoveAreaRadius - BallRadius);
